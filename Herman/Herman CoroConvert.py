    ...
    if filenames == ():
        ...
        if directory == '':
            ...
            for filename in filenames:  
                ...
                # Numpy Array Output /////////////
                np.save(filename, img_array_limited_equalized)
                
                i = 0
                w = np.empty([512, 512])
                for arr in img_array_limited_equalized:
                    w += arr
                    i +=1
                v = w/i
                
                np.save(filename+'_average', v)
...
button_load=ttk.Button(root, text='Load', width=20, command=loadButton)
button_load.place(x=380, y=565)

button_convert=ttk.Button(root, text='To Array', width=20, command=toArrayButton)
